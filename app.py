...
def index():
    ...
    user_info = db.execute("SELECT symbol, amount FROM amounts WHERE user_id = ?;",
                           user_id)
    ...
    return render_template("index.html", user_info=user_info, user_cash=user_cash, net_worth=net_worth)
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?);
                    """,
                   user_id, symbol, result["price"], shares, 'buy', date
                   )

        # Add purchase to amounts table (or ignore if already exists)
        db.execute("INSERT OR IGNORE INTO amounts (user_id, symbol, amount) VALUES (?, ?, ?)", user_id, symbol, 0)
        db.execute("UPDATE OR IGNORE amounts SET amount = amount + ? WHERE user_id = ? AND symbol = ?", shares, user_id, symbol)
...
@login_required
def sell():
    # Set user_id to be visible for both GET and POST
    user_id = session["user_id"]
    ...
    # User accessed via POST
        ...
        q = db.execute("""SELECT amount FROM amounts
                       WHERE user_id = ?
                       AND symbol = ?;""",
                       user_id, symbol)
